# -*- coding: utf-8 -*-
"""
Created on Wed Mar  5 21:22:35 2025
        ...
        with ThreadPoolExecutor(max_workers=self.rudder_worker_num, 
            ...
            while self.simulation_running:
                ...
                if self.waypoints[0][0] is not None:
                    ...
                    if self.distance_to_waypoint <= 0.25 and self.waypoint_index < len(self.waypoints)-1:
...
# =============================================================================    
        ...
        with ThreadPoolExecutor(max_workers=self.elevator_worker_num, 
            ...
            while self.simulation_running:
                ...
                if self.waypoint_index < len(self.waypoints) - 2 or (self.waypoint_index >= len(self.waypoints) - 3 and self.distance_to_waypoint >= 1.4):
                    ...
                    lt = LOS + 45 # - self.terminal_pitch_target
                    lp = LOS - pitch
                    pitch_err = lt + lp
    ...
    def calculate_bezier_curve(self, initial_pos, target_pos):
        ...
        P0 = initial_pos
        P1 = (initial_pos[0], initial_pos[1] + (target_pos[1] - initial_pos[1])*0.9)
        P2 = (target_pos[0] + (target_pos[1] - initial_pos[1])*0.1, target_pos[1])
        P3 = target_pos
    ...
    def get_observation(self):
        ...
        while self.simulation_running:
            try:
                ...
                if self.target_latitude == None and self.target_longitude == None:
                    ...
                    if self.get_direction(yaw) in ['N', 'S']:
                        true_yaw = (yaw + 180) % 180
                        
                        lat_gap = math.cos(math.radians(90-true_yaw)) * STRAIGHT_DIST
                        long_gap = math.cos(math.radians(true_yaw)) * STRAIGHT_DIST
                        
                        diameter_lat_gap = math.cos(math.radians(true_yaw)) * DIAMETER_TURNING_POINT
                        diameter_long_gap = math.cos(math.radians(90-true_yaw)) * DIAMETER_TURNING_POINT
    ...
    def save_data_to_csv(self):
        ...
        black_box_filename = f"terminal_angle/flight_data/black_box_{self.timestr}.xlsx"
        ...
        action_history_filename = f"terminal_angle/flight_data/action_history_{self.timestr}.xlsx"
        mode = 'w' if self.ep == 0 else 'a'
