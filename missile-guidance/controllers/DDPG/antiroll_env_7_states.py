...
class AntirollEnv(gym.Env):
    ...
    def step(self, action_space):
        ...
        self.state_history.append(self.black_box[-1] + tuple([self.current_reward])) # if there are any data repeated or the length is similar to the black box then remove this
    ...
    def calculate_rewards(self, state, action):
        ...
        # shaping = shaping + STABLE_REWARD * (1 / abs(roll))
        
        # reward if it can keep the roll between -5 and 5 (C = 5) for the last 10 steps        
...
##################################################################################
    ...
    def get_observation(self):
        ...
        while self.simulation_running:
            try:
                ...
                self.state_stack.append([[roll, roll_rate, pitch, pitch_rate, 
                                          yaw, yaw_rate, velocity], is_grounded])
                ...
                is_grounded = True
                self.state_stack.append([[0, 0, 0, 0, 0, 0, 0], is_grounded])
    ...
    def save_data_to_csv(self):
        ...
        black_box_filename = f"ddpg/new_training/black_box_rl/black_box_rl_{self.timestr}.xlsx"
        if self.episode == 1:
        ...
        state_history_values = pd.DataFrame(self.state_history, 
                                            columns=['pos_x', 'pos_y', 'pos_z', 'altitude', 
                                                     'velocity', 'ver_vel', 'roll', 'roll_rate',
                                                     'pitch', 'pitch_rate', 'yaw', 'yaw_rate', 'current_reward'])

        state_history_filename = f"ddpg/new_training/black_box_rl/state_history_{self.timestr}.xlsx"
        if self.episode == 1:
        ...
        action_history_filename = f"ddpg/new_training/black_box_rl/action_history_{self.timestr}.xlsx"
        if self.episode == 1:
