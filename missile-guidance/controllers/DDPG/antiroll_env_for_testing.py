# -*- coding: utf-8 -*-
"""
Created on Sat Nov  2 12:17:32 2024
...
from ddpg.common_definitions import A, B, C, D
...
class AntirollEnv(gym.Env):
    ...
    def __init__(self, max_episode=1, IP="localhost", main_port=2873, 
        ...
        self.saturation = 1.0   # TODO: observe this
        self.action_pwm = [None]  # should this be None or 0?
        # belong to the observator thread
        self.black_box = [] # belong to the observator thread
        self.state = []
        self.state_history = []
        self.previous_state = None
        self.stable_roll = deque(maxlen=20)
    ...
    def step(self, action_space):
        ...
        roll = self.state[0]
        roll_rate = self.state[1]
        print(f'\nroll: {roll}\nroll rate: {roll_rate}\n')
        
        # if abs(roll) > 3.5:
            # perform the action
            # self.action_pwm = action_space
    ...
    def calculate_rewards(self, state, action):
        ...
        shaping = shaping - abs(roll*B) - abs(roll_rate*B)
    ...
    def close(self):
        # self.observator.join()
        self.aileron_semaphore.release()
        # self.aileron_thread_1.join()
        self.aileron_semaphore.release()
        # self.aileron_thread_2.join()
        
        if self.use_elev_and_rudd:
            # self.altitude_evaluator_thread.join()
            self.elevator_semaphore.release()
            # self.elevator_thread_1.join()
            self.elevator_semaphore.release()
            # self.elevator_thread_2.join()
