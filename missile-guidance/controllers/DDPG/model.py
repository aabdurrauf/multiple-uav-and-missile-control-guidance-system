...
https://github.com/samkoesnadi/DDPG-tf2/blob/master/src/model.py
...
from controllers.DDPG.common_definitions import (
    KERNEL_INITIALIZER, GAMMA, RHO,
    STD_DEV, BUFFER_SIZE, BATCH_SIZE,
    CRITIC_LR, ACTOR_LR, ACTOR_NODES_1,
    ACTOR_NODES_2, CRITIC_NODES_1, CRITIC_NODES_2,
    CRITIC_NODES_3, CRITIC_NODES_4
    
)
from controllers.DDPG.buffer import ReplayBuffer
from controllers.DDPG.utils import OUActionNoise
...
def ActorNetwork(num_states=24, num_actions=4, action_high=1):
    ...
    out = tf.keras.layers.Dense(ACTOR_NODES_1, activation=tf.nn.relu, # tf.nn.leaky_relu,
                                kernel_initializer=KERNEL_INITIALIZER)(inputs)
    # ammar added batch norm layer
    out = tf.keras.layers.BatchNormalization()(out)
    ### ----
    out = tf.keras.layers.Dense(ACTOR_NODES_2, activation=tf.nn.relu, # tf.nn.leaky_relu,
...
def CriticNetwork(num_states=24, num_actions=4, action_high=1):
    ...
    state_out = tf.keras.layers.Dense(CRITIC_NODES_1, activation=tf.nn.relu, # tf.nn.leaky_relu,
                                      kernel_initializer=KERNEL_INITIALIZER)(state_input)
    state_out = tf.keras.layers.BatchNormalization()(state_out)
    state_out = tf.keras.layers.Dense(CRITIC_NODES_2, activation=tf.nn.relu, # tf.nn.leaky_relu,
                                      kernel_initializer=KERNEL_INITIALIZER)(state_out)

    # Action as input
    action_input = tf.keras.layers.Input(shape=(num_actions), dtype=tf.float32)
    action_out = tf.keras.layers.Dense(CRITIC_NODES_3, activation=tf.nn.relu, # tf.nn.leaky_relu,
    ...
    outs = tf.keras.layers.Dense(CRITIC_NODES_4, activation=tf.nn.relu, # tf.nn.leaky_relu,
...
class Brain:  # pylint: disable=too-many-instance-attributes
    ...
    def save_weights(self, path):
        ...
        # Save the weights
        self.actor_network.save_weights(path + "-actor_network.h5")
        self.critic_network.save_weights(path + "-critic_network.h5")
    ...
    def load_weights(self, path):
        ...
        """
        try:
            self.actor_network.load_weights(path + "-actor_network.h5")
            self.critic_network.load_weights(path + "-critic_network.h5")
